...
```{r saving figures, eval=FALSE, echo=TRUE}
...
```{r solution ggplot mean errorbars, eval=FALSE, echo = FALSE}
# Solution 
#Create an object with dplyr
mean_sd_metadata <- metadata %>%
  group_by(cit) %>%
  summarize(mean_size = mean(genome_size, na.rm = TRUE), 
            sd_size = sd(genome_size, na.rm = TRUE))

# Looking at the structure and data type of mean_sd_metadata
class(mean_sd_metadata)
glimpse(mean_sd_metadata)

ggplot(mean_sd_metadata, aes(x = cit, y= mean_size, shape = cit, color = cit)) +
  geom_point(size = rel(3.0)) +
  geom_errorbar(width = 0.1, aes(ymax = mean_size + sd_size, ymin = mean_size - sd_size)) + 
  theme(axis.text.x = element_text(angle=45, hjust=1))
```
...
```{r ggplot facet, eval=FALSE, echo = TRUE}
# Break up 3 groups 
ggplot(metadata, aes(x = sample, y= genome_size, color = generation, shape = cit)) +
  geom_point(size = rel(3.0)) +
  theme(axis.text.x = element_text(angle=45, hjust=1)) + 
...
```{r solution challenge, eval=FALSE, echo = FALSE}
#1.  Create a faceted plot of sample (x-axis) and genome size (y-axis) broken up by **clade**.  Make the colors of the plot to represent each clade. 
...
# 2.  Create a third faceted plot with the clade (x-axis) and mean genome size on the (y- axis) and 
mean_clade_metadata <- metadata %>%
  group_by(clade) %>%
  summarize(mean_size = mean(genome_size, na.rm = TRUE), 
            sd_size = sd(genome_size, na.rm = TRUE))

ggplot(mean_clade_metadata, aes(x = clade, y= mean_size, color = clade)) +
  geom_point(size = rel(3.0)) +
  theme(axis.text.x = element_text(angle=45, hjust=1)) + 
  facet_grid(. ~ clade, scales = "free_x") + 
  geom_errorbar(width = 0.1, aes(ymax = mean_size + sd_size, ymin = mean_size - sd_size))
...
# 3.  Create another facetted plot with sample (x-axis) and genome size (y-axis) broke up by **generation**.  Color the points by the citrate mutant.  What does this tell us about what "unknown" means? 
ggplot(metadata, aes(x = sample, y= genome_size, color = cit)) +
  geom_point(size = rel(3.0)) +
  theme(axis.text.x = element_text(angle=45, hjust=1)) + 
